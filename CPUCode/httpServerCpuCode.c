...
static void exchangeItems(int sock, const uint8_t* items, uint16_t num_items) {
	...
	printf(
			"Number of send sendCount = %u, Number of received recvCount = %u\n",
			sendCount, recvCount);
...
void writeDataToLMem(uint64_t *dataIn, int size, int sizeBytes,
		int burstLengthInBytes, max_engine_t *engine, max_file_t *maxfile) {

	max_actions_t *actions = max_actions_init(maxfile, NULL);

	max_set_ticks(actions, "KernelLMem_Write_CommandAndDataStream", size);
	max_set_uint64t(actions, "KernelLMem_Write_CommandAndDataStream",
			"totalBursts", size * 8 / burstLengthInBytes);
	max_set_uint64t(actions, "KernelLMem_Write_CommandAndDataStream",
			"wordsPerBurst", burstLengthInBytes / 8);
	max_set_ticks(actions, "KernelLMem_Read_CommandAndDataStream", 0);
	max_set_uint64t(actions, "KernelLMem_Read_CommandAndDataStream",
			"totalBursts", size * 8 / burstLengthInBytes);
	max_set_uint64t(actions, "KernelLMem_Read_CommandAndDataStream",
			"wordsPerBurst", burstLengthInBytes / 8);
...
int main(int argc, char *argv[]) {
	...
	char fileHashIndex1[] =
			"/home/mvorkapic/workspace/working/CPUCode/romHashIndex1_init.html";
	char fileHashIndex2[] =
			"/home/mvorkapic/workspace/working/CPUCode/romHashIndex2_init.html";
	...
	for (uint32_t i = 0; i < romDepthHash; i++) {
		...
		max_set_mem_uint64t(actions, "hashIndex", "romHashIndex1", i,
				arrHash1[i]);
		max_set_mem_uint64t(actions, "hashIndex", "romHashIndex2", i,
				arrHash2[i]);
	}
	...
	char file[] =
			"/home/mvorkapic/workspace/working/CPUCode/lmem_generated_file.html";
	...
	max_ip_config(engine, MAX_NET_CONNECTION_QSFP_BOT_10G_PORT1, &dfe_ip,
			&netmask); //MAX_NET_CONNECTION_CH2_SFP1
	...
	for (int i = 0; i < Nsockets; i++) {
		dfe_socket[i] = max_tcp_create_socket(engine,
				"tcp_ISCA_QSFP_BOT_10G_PORT1"); //socket 0 //"tcp_ch2_sfp1"
	...
	posix_memalign((void *) &read_buffer, 16, 16 * 1);
	read_llstream = max_llstream_setup(engine, "toCpuByteNumber", 1, 16,
			read_buffer);
	...
	read_llstreamStateMachine = max_llstream_setup(engine,
			"toCpuByteNumberStateMachine", 1, 16, read_bufferStateMachine);

	size_t txMgr_buffer_size = 512 * sizeof(tx_event_type_t);
	void *txMgr_buffer;
	posix_memalign(&txMgr_buffer, 4096, txMgr_buffer_size);
	max_llstream_t *llstream = max_llstream_setup(engine, "toCpuOutputTxEvent",
			512, sizeof(tx_event_type_t), txMgr_buffer);
	void *txMgrSlot;
	...
	while (1) {
		...
		while (exitFlagw == 0) {
			...
			if ((frameCounter - frameCounterPrevious) == y) {
				...
				sleep(2);
				printf(
						"CPU code: frameCounter=%ld, frameCounterPrevious=%ld, ceil(fileLength)=%u\n",
				...
				max_tcp_await_state(dfe_socket[socket_returned],
						MAX_TCP_STATE_CLOSED, NULL);
				//dfe_socket[0] = max_tcp_create_socket(engine, "tcp_ISCA_QSFP_BOT_10G_PORT1");  //socket 0 //"tcp_ch2_sfp1"
				...
				max_tcp_await_state(dfe_socket[socket_returned],
						MAX_TCP_STATE_LISTEN, NULL); //ceka dok tcp SOCKET ne dodje u LISTEN connection state
	...
	int vrednost = connect(cpu_socket, (const struct sockaddr*) &cpu,
			sizeof(cpu));
	...
	//static const
	uint8_t items5[] = { 0x47, 0x45, 0x54, 0x20, 0x2f, 0x20, /* GET /  */
