	...
	void *read_ptr_socket_slot;
		...
		while(1)
		{
			for(int i=0;i<Nsockets;i++)
			{
				...
				printf("CPU code: time=%i, socket=%i, max_tcp_get_num_bytes_received=%llu, max_tcp_get_num_bytes_transmitted=%llu\n", ti, i, (long long unsigned int) num_rx_bytes, (long long unsigned int) num_tx_bytes);
			}
			ti++;
			printf("\n");
			usleep(1000*1000);
	...
	while (1) {
		...
		while (FoundByteNumber != 1) //first wait to receive LengthBytes number
		{
			...
			if (ii) {
				...
				printf("CPU code: fileSizeBytes=%u\n", (unsigned int) *byteNumber);
		...
		while (1) {
			...
			if (num_tx_bytes == fileBytes) {
				//usleep(1000*1000*3);
				printf("CPU code: num_tx_bytes==fileBytes, socket_returned=%u, fileBytes=%u, num_tx_bytes(max_tcp_get_num_bytes_transmitted)=%llu\n", socket_returned, fileBytes, (long long unsigned int) num_tx_bytes);
